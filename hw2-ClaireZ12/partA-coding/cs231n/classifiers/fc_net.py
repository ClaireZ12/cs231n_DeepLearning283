...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # layer, etc.                                                              #
        ############################################################################
        #define function 'affine_bn_relu_forward' in file 'layer_utils.py', and show it as follows
        ...
        # of 0.5 to simplify the expression for the gradient.                      #
        ############################################################################
        # define function 'affine_bn_relu_backward' in file 'layer_utils.py', and show it as follows
        ...
        for i in range(self.num_layers, 0, -1):
            if i == self.num_layers:
                dhiddens[i-1], grads['W%d'%(i)], grads['b%d'%(i)] = affine_backward(dhiddens[i], caches[i-1])
            else:
                if self.use_dropout:
                    dhiddens[i] = dropout_backward(dhiddens[i], dp_caches[i-1])
                if self.normalization=='batchnorm':
                    dx, dw, db, dgamma, dbeta = affine_bn_relu_backward(dhiddens[i], caches[i-1])
                    dhiddens[i-1], grads['W%d'%(i)], grads['b%d'%(i)] = dx, dw, db
